...
class DomainExpert():
    ...
    def _constructParameters(self, parameters: List[up.model.Parameter], 
    ...
    def _constructTree(self, fnode: up.model.fnode.FNode, nodes: List[msg.Node], 
        ...
        if fnode.node_type == OperatorKind.FLUENT_EXP:
            ...
            node.parameters = self._constructParameters(parameters, params_map)

        else:
            negate = fnode.node_type == OperatorKind.NOT
            for child_fnode in fnode.args:
                child_node = self._constructTree(child_fnode, nodes, negate=negate, params_map=params_map)
                node.children.append(child_node.node_id)

        return node

    def _constructEffectsTree(self, effects: List[up.model.effect.Effect], 
        ...
        for effect in effects:
            if effect.value.is_bool_constant():
                ...
                fluent_node = self._constructTree(effect.fluent, nodes, params_map=params_map)
                ...
                fluent_node = self._constructTree(effect.fluent, nodes, params_map=params_map)
                value_node = self._constructTree(effect.value, nodes, params_map=params_map)
    ...
    def getAction(self, action_name: str, parameters: List[str]) -> msg.Action:
        ...
        params_map = dict([(p.name, parameters[i].lower() if i<len(parameters) else f'?{p.type.name}{i}') for i,p in enumerate(action.parameters)])
        action_msg.parameters = self._constructParameters(action.parameters, params_map)
        ...
        for precondition in action.preconditions:
            node = self._constructTree(precondition, action_msg.preconditions.nodes, params_map=params_map)
            and_node.children.append(node.node_id)

        action_msg.effects = msg.Tree()
        action_msg.effects.nodes = list()
        self._constructEffectsTree(action.effects, action_msg.effects.nodes, params_map)
        return action_msg

    def getDurativeActions(self) -> List[str]:
        return list(map(lambda a: a.name, self.domain.durative_actions))

    def getDurativeAction(self, action_name: str, parameters: List[str]) -> msg.DurativeAction:
        try:
            durative_action = self.domain.action(action_name.lower())
            if not isinstance(durative_action, DurativeAction):
                return None
        except UPValueError:
            return None
        ...
        params_map = dict([(p.name, parameters[i].lower() if i<len(parameters) else f'?{p.type.name}{i}') for i,p in enumerate(durative_action.parameters)])
        durative_action_msg.parameters = self._constructParameters(durative_action.parameters, params_map)
        ...
        for time_interval, conditions in durative_action.conditions.items():
            if time_interval.lower == timing.StartTiming() and time_interval.upper == timing.EndTiming():
                ...
                for condition in conditions:
                    node = self._constructTree(condition, durative_action_msg.over_all_requirements.nodes, params_map=params_map)
                    and_node.children.append(node.node_id)

            elif time_interval.lower == timing.StartTiming():
                and_node = and_nodes[1]
                for condition in conditions:
                    node = self._constructTree(condition, durative_action_msg.at_start_requirements.nodes, params_map=params_map)
                    and_node.children.append(node.node_id)

            elif time_interval.upper == timing.EndTiming():
                and_node = and_nodes[2]
                for condition in conditions:
                    node = self._constructTree(condition, durative_action_msg.at_end_requirements.nodes, params_map=params_map)
                    and_node.children.append(node.node_id)
        
        for time, effects in durative_action.effects.items():
            if time == timing.StartTiming():
                self._constructEffectsTree(effects, durative_action_msg.at_start_effects.nodes, params_map)
            else:
                self._constructEffectsTree(effects, durative_action_msg.at_end_effects.nodes, params_map)
    ...
    def getPredicate(self, predicate_name: str) -> msg.Node:
        ...
        for predicate in predicates:
            if predicate.name == predicate_name.lower():
                ...
                predicate_msg.parameters = self._constructParameters(predicate.signature, params_map)
    ...
    def getFunction(self, function_name: str) -> msg.Node:
        ...
        for function in functions:
            if function.name == function_name.lower():
                ...
                function_msg.parameters = self._constructParameters(function.signature, params_map)
